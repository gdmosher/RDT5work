...
#    image_dir <- paste0(outpath, "/", image_dir)      #old way
    image_dir <- paste0("../../images/mydoc/", image_dir)   #for v6
    image_dir <- gsub("/{1,}", "/", image_dir)
    dir.create(image_dir, showWarnings=FALSE)     #is this working?
...
#v6                               "../", 
    ...
    for(i in seq_along(mdlist)) {
        ...
        endmatter <- c(tg_class='<div class="tags">',
                       ...
                       tg_href=paste0('<a href="mydoc_', outfilebasename, "_", filenumbers[next_page], '" class="btn btn-default navbar-btn cursorNorm" role="button">next_page</a>'),
                       tg_endDiv="</div>")
...
#        sidebartitle <- mymaindoctitle     #try to fix short titles in sidebar, messed up sidebar "remove existing"
    } else {
